...
;(function() {
    ...
    evolu.Code.prototype = {
        ...
        run: function() {
            ...
            var line
            for (var id in this._running) {
                this.currentRule = this._running[id]
                for (var i = 0; i < this.currentRule.lines.length; i++) {
                    line = this.currentLine = this.currentRule.lines[i]
                    if (line.command.run)
